import constants
from sqlalchemy import create_engine, Table, Column, Integer, String, MetaData, update, select, or_
from constants import VK_MESSENGER, TELEGRAM_MESSENGER
...
def save_user_id(user_id, type_of_messenger):
    ...
    if type_of_messenger == VK_MESSENGER['messenger_name']:
        for row in rows:
            if row[1] != user_id:
                i += 1
    ...
    elif type_of_messenger == TELEGRAM_MESSENGER['messenger_name']:
        for row in rows:
            if row[0] != user_id:
                i += 1
            if rowcount - i == 0:
